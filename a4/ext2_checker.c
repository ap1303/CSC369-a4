        ...
        if (block != 0) {
            ...
            while (rec_len_sum < EXT2_BLOCK_SIZE) {
               ...
               if (dir -> file_type == EXT2_FT_REG_FILE) {
                   fix_file(sb, bg, inode_bitmap, block_bitmap, dir, inode_table + ((dir -> inode) - 1), dir -> inode, &total_fixes);
               } else if (dir -> file_type == EXT2_FT_SYMLINK) {
                   fix_symlink_files(sb, bg, inode_bitmap, block_bitmap, dir, inode_table + ((dir -> inode) - 1), dir -> inode, &total_fixes);
               } else {
                   if (strcmp(dir -> name, ".") != 0 && strcmp(dir -> name, "..") != 0) {
                       fix_dir_files(disk, sb, bg, inode_bitmap, block_bitmap, inode_table, dir, inode_table + ((dir -> inode) - 1), dir -> inode, &total_fixes);
                   } 
               }
